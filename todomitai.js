...
console.log('Iniciando conexiones a domitai');
...
const wallet = new bitcoin_core({ 
    port: process.env.RDD_PORT, 
    username: process.env.RDD_USERNAME, 
    password: process.env.RDD_PASSWORD });
    ...
    /*
        ...
        while((tx=await redis.rpoplpushAsync(qTRADES_RDDBTC, qTRADES_RDDBTC+':w'))) {
            ...
            if(o.result.QuantityRemaining===0) { // Convertir BTC->LTC
                ...
                const btc_ltc = await bittrex.gettickerAsync({market:'BTC-LTC'});
                if((quantity/btc_ltc.result.Ask*0.998)*btc_ltc.result.Ask>=0.001) {
					console.log({
						market: 'BTC-LTC',
						quantity: (quantity/btc_ltc.result.Ask*0.998).toFixed(8),
						rate: btc_ltc.result.Ask,
						timeInForce: 'GTC',
						resultante: ((quantity/btc_ltc.result.Ask*0.998)*btc_ltc.result.Ask).toFixed(8)
					});
